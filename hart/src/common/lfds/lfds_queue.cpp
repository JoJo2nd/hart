...
struct lfds_queue_state {
  struct lfds_queue_element* volatile enqueue[LFDS_QUEUE_PAC_SIZE], *volatile dequeue[LFDS_QUEUE_PAC_SIZE];

  lfds_atom_t aba_counter;
...
struct lfds_queue_element {
  ...
  struct lfds_queue_element* volatile next[LFDS_QUEUE_PAC_SIZE];
...
void lfds_queue_internal_new_element_from_freelist(struct lfds_queue_state*   qs,
                                                   struct lfds_queue_element* qe[LFDS_QUEUE_PAC_SIZE], void* user_data);
void lfds_queue_internal_guaranteed_new_element_from_freelist(struct lfds_queue_state*   qs,
                                                              struct lfds_queue_element* qe[LFDS_QUEUE_PAC_SIZE],
                                                              void*                      user_data);
void lfds_queue_internal_init_element(struct lfds_queue_state* qs, struct lfds_queue_element* qe[LFDS_QUEUE_PAC_SIZE],
                                      struct lfds_freelist_element* fe, void* user_data);
...
int lfds_queue_new(struct lfds_queue_state** qs, lfds_atom_t number_elements) {
  ...
  *qs =
    (struct lfds_queue_state*)lfds_liblfds_aligned_malloc(sizeof(struct lfds_queue_state), LFDS_ALIGN_DOUBLE_POINTER);

  if (*qs != NULL) {
    // TRD : the size of the lfds_freelist is the size of the lfds_queue (+1 for
    // the leading dummy element, which is hidden from the caller)
...
void lfds_queue_internal_new_element_from_freelist(struct lfds_queue_state*   qs,
  ...
  if (fe != NULL) lfds_queue_internal_init_element(qs, qe, fe, user_data);

  return;
}

/****************************************************************************/
void lfds_queue_delete(struct lfds_queue_state* qs,
  ...
  // TRD : leading load barrier not required as it will be performed by the
  // dequeue

  while (lfds_queue_dequeue(qs, &user_data))
    if (user_data_delete_function != NULL) user_data_delete_function(user_data, user_state);
...
void lfds_queue_query(struct lfds_queue_state* qs, enum lfds_queue_query_type query_type, void* query_input,
                      void* query_output) {
  assert(qs != NULL);
...
int lfds_queue_enqueue(struct lfds_queue_state* qs, void* user_data) {
  ...
  if (qe[LFDS_QUEUE_POINTER] == NULL) return (0);
...
/****************************************************************************/
// int lfds_queue_guaranteed_enqueue( struct lfds_queue_state *qs, void
// *user_data )
// {
...
void lfds_queue_internal_queue(struct lfds_queue_state* qs, struct lfds_queue_element* qe[LFDS_QUEUE_PAC_SIZE]) {
  LFDS_ALIGN(LFDS_ALIGN_DOUBLE_POINTER)
  struct lfds_queue_element *enqueue[LFDS_QUEUE_PAC_SIZE], *next[LFDS_QUEUE_PAC_SIZE];
  ...
  // TRD : the DCAS operation issues a read and write barrier, so we don't need
  // a read barrier in the do() loop
  ...
  do {
    ...
    */
    ...
    if (qs->enqueue[LFDS_QUEUE_POINTER] == enqueue[LFDS_QUEUE_POINTER] &&
        qs->enqueue[LFDS_QUEUE_COUNTER] == enqueue[LFDS_QUEUE_COUNTER]) {
      if (next[LFDS_QUEUE_POINTER] == NULL) {
        qe[LFDS_QUEUE_COUNTER] = next[LFDS_QUEUE_COUNTER] + 1;
        cas_result = lfds_abstraction_dcas((volatile lfds_atom_t*)enqueue[LFDS_QUEUE_POINTER]->next, (lfds_atom_t*)qe,
                                           (lfds_atom_t*)next);
      } else {
...
int lfds_queue_dequeue(struct lfds_queue_state* qs, void** user_data) {
  LFDS_ALIGN(LFDS_ALIGN_DOUBLE_POINTER)
  struct lfds_queue_element *enqueue[LFDS_QUEUE_PAC_SIZE], *dequeue[LFDS_QUEUE_PAC_SIZE], *next[LFDS_QUEUE_PAC_SIZE];

  unsigned char cas_result = 0;

  int rv = 1, state = LFDS_QUEUE_STATE_UNKNOWN, finished_flag = LOWERED;
  ...
  do {
    ...
    */
    ...
    if (dequeue[LFDS_QUEUE_POINTER] == qs->dequeue[LFDS_QUEUE_POINTER] &&
        dequeue[LFDS_QUEUE_COUNTER] == qs->dequeue[LFDS_QUEUE_COUNTER]) {
      ...
      if (enqueue[LFDS_QUEUE_POINTER] != dequeue[LFDS_QUEUE_POINTER]) state = LFDS_QUEUE_STATE_ATTEMPT_DELFDS_QUEUE;
      ...
      case LFDS_QUEUE_STATE_ATTEMPT_DELFDS_QUEUE:
        *user_data = next[LFDS_QUEUE_POINTER]->user_data;
        ...
        cas_result =
          lfds_abstraction_dcas((volatile lfds_atom_t*)qs->dequeue, (lfds_atom_t*)next, (lfds_atom_t*)dequeue);

        if (cas_result == 1) finished_flag = RAISED;
        break;
      }
    }
  } while (finished_flag == LOWERED);

  if (cas_result == 1) lfds_freelist_push(qs->fs, dequeue[LFDS_QUEUE_POINTER]->fe);
...
void lfds_queue_internal_init_element(struct lfds_queue_state* qs, struct lfds_queue_element* qe[LFDS_QUEUE_PAC_SIZE],
  ...
  qe[LFDS_QUEUE_POINTER]->next[LFDS_QUEUE_COUNTER] =
    (struct lfds_queue_element*)lfds_abstraction_increment((lfds_atom_t*)&qs->aba_counter);
