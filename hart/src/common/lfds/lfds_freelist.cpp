/********************************************************************
    Written by James Moran but based on work at http://liblfds.org/
    Please see the file LICENSE.txt in the repository root directory.
*********************************************************************/

#include "hart/lfds/lfds.h"
#include "lfds/lfds_utils.inl"
...
struct lfds_freelist_state {
  struct lfds_freelist_element* volatile top[LFDS_FREELIST_PAC_SIZE];

  int (*user_data_init_function)(void** user_data, void* user_state);

  void* user_state;

  lfds_atom_t aba_counter, element_count;
};

struct lfds_freelist_element {
  struct lfds_freelist_element* next[LFDS_FREELIST_PAC_SIZE];
...
/****************************************************************************/
int lfds_freelist_new(struct lfds_freelist_state** fs, lfds_atom_t number_elements,
                      int (*user_data_init_function)(void** user_data, void* user_state), void* user_state) {
  int rv = 0;

  lfds_atom_t element_count;

  assert(fs != NULL);
  // TRD : number_elements can be any value in its range
  // TRD : user_data_init_function can be NULL

  *fs = (struct lfds_freelist_state*)lfds_liblfds_aligned_malloc(sizeof(struct lfds_freelist_state),
                                                                 LFDS_ALIGN_DOUBLE_POINTER);
  ...
  if ((*fs) != NULL) {
    ...
    if (element_count == number_elements) rv = 1;
...
lfds_atom_t lfds_freelist_internal_new_element(struct lfds_freelist_state* fs, struct lfds_freelist_element** fe) {
  ...
  *fe = (struct lfds_freelist_element*)lfds_liblfds_aligned_malloc(sizeof(struct lfds_freelist_element),
                                                                   LFDS_ALIGN_DOUBLE_POINTER);
  ...
  if (rv == 1) lfds_abstraction_increment((lfds_atom_t*)&fs->element_count);
...
struct lfds_freelist_element* lfds_freelist_pop(struct lfds_freelist_state* fs, struct lfds_freelist_element** fe) {
  LFDS_ALIGN(LFDS_ALIGN_DOUBLE_POINTER)
  struct lfds_freelist_element* fe_local[LFDS_FREELIST_PAC_SIZE];
  ...
  /* TRD : note that lfds_abstraction_dcas loads the original value of the
     destination (fs->top) into the compare (fe_local)
           (this happens of course after the CAS itself has occurred inside
     lfds_abstraction_dcas)
  */
  ...
  } while (0 == lfds_abstraction_dcas((volatile lfds_atom_t*)fs->top,
                                      (lfds_atom_t*)fe_local[LFDS_FREELIST_POINTER]->next, (lfds_atom_t*)fe_local));
...
void lfds_freelist_push(struct lfds_freelist_state* fs, struct lfds_freelist_element* fe) {
  LFDS_ALIGN(LFDS_ALIGN_DOUBLE_POINTER)
  struct lfds_freelist_element *fe_local[LFDS_FREELIST_PAC_SIZE], *original_fe_next[LFDS_FREELIST_PAC_SIZE];
  ...
  fe_local[LFDS_FREELIST_COUNTER] =
    (struct lfds_freelist_element*)lfds_abstraction_increment((lfds_atom_t*)&fs->aba_counter);
  ...
  /* TRD : note that lfds_abstraction_dcas loads the original value of the
     destination (fs->top) into the compare (original_fe_next)
           (this happens of course after the CAS itself has occurred inside
     lfds_abstraction_dcas)
           this then causes us in our loop, should we repeat it, to update
     fe_local->next to a more
  ...
  */
  ...
  do {
    ...
    0 == lfds_abstraction_dcas((volatile lfds_atom_t*)fs->top, (lfds_atom_t*)fe_local, (lfds_atom_t*)original_fe_next));
...
void* lfds_freelist_get_user_data_from_element(struct lfds_freelist_element* fe, void** user_data) {
  ...
  if (user_data != NULL) *user_data = fe->user_data;
...
/****************************************************************************/
void lfds_freelist_delete(struct lfds_freelist_state* fs,
                          void (*user_data_delete_function)(void* user_data, void* user_state), void* user_state) {
  struct lfds_freelist_element* fe;
...
void lfds_freelist_internal_validate(struct lfds_freelist_state* fs, struct lfds_validation_info* vi,
                                     enum lfds_data_structure_validity* lfds_freelist_validity) {
  struct lfds_freelist_element *fe, *fe_slow, *fe_fast;

  lfds_atom_t element_count = 0;
  ...
  */
  ...
  if (fe_slow != NULL) do {
      ...
      if (fe_fast != NULL) fe_fast = fe_fast->next[LFDS_FREELIST_POINTER];

      if (fe_fast != NULL) fe_fast = fe_fast->next[LFDS_FREELIST_POINTER];
    } while (fe_slow != NULL && fe_fast != fe_slow);

  if (fe_fast != NULL && fe_slow != NULL && fe_fast == fe_slow) *lfds_freelist_validity = LFDS_VALIDITY_INVALID_LOOP;
  ...
  */
  ...
  if (*lfds_freelist_validity == LFDS_VALIDITY_VALID && vi != NULL) {
    ...
    if (element_count < vi->min_elements) *lfds_freelist_validity = LFDS_VALIDITY_INVALID_MISSING_ELEMENTS;

    if (element_count > vi->max_elements) *lfds_freelist_validity = LFDS_VALIDITY_INVALID_ADDITIONAL_ELEMENTS;
  }

  return;
}

void lfds_freelist_query(struct lfds_freelist_state* fs, enum lfds_freelist_query_type query_type, void* query_input,
                         void* query_output) {
  assert(fs != NULL);
  ...
  case LFDS_FREELIST_QUERY_VALIDATE:
    ...
    lfds_freelist_internal_validate(fs, (struct lfds_validation_info*)query_input,
                                    (enum lfds_data_structure_validity*)query_output);
    break;
  }
