...
// Don't remove this file from your project! It is useful reference code that
// you can execute.
// You can call ImGui::ShowTestWindow() in your code to learn about various
// features of ImGui.
...
void ImGui::ShowTestWindow(bool* p_open) {
  ...
  if (show_app_about) {
    ...
    ImGui::Text("ImGui is licensed under the MIT License, see LICENSE for more "
                "information.");
    ImGui::End();
  }
  ...
  if (ImGui::CollapsingHeader("Help")) {
    ImGui::TextWrapped("This window is being created by the ShowTestWindow() "
  ...
  if (ImGui::CollapsingHeader("Window options")) {
    ...
    if (ImGui::TreeNode("Logging")) {
      ImGui::TextWrapped("The logging API redirects all text output so you can "
                         "easily capture the content of a window or a block. "
                         "Tree nodes can be automatically expanded. You can "
                         "also call ImGui::LogText() to output directly to the "
  ...
  if (ImGui::CollapsingHeader("Widgets")) {
    if (ImGui::TreeNode("Trees")) {
      ...
      if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
        ShowHelpMarker("This is a more standard looking tree with selectable "
                       "nodes.\nClick to select, CTRL+Click to toggle, click "
                       "on arrows or double-click to open.");
        static bool align_label_with_current_x_position = false;
        ImGui::Checkbox("Align label with current X position)", &align_label_with_current_x_position);
        ImGui::Text("Hello!");
        if (align_label_with_current_x_position) ImGui::Unindent(ImGui::GetTreeNodeToLabelSpacing());

        static int selection_mask = (1 << 2); // Dumb representation of what may be user-side selection
                                              // state. You may carry selection state inside or outside
                                              // your objects in whatever format you see fit.
        int node_clicked = -1;                // Temporary storage of what node we have clicked
                                              // to process selection at the end of the loop.
                                              // May be a pointer to your own node type, etc.
        ImGui::PushStyleVar(ImGuiStyleVar_IndentSpacing, ImGui::GetFontSize() * 3); // Increase spacing to
    ...
    if (ImGui::TreeNode("Word Wrapping")) {
      ...
      ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(pos.x + wrap_width, pos.y),
                                                ImVec2(pos.x + wrap_width + 10, pos.y + ImGui::GetTextLineHeight()),
                                                0xFFFF00FF);
      ImGui::PushTextWrapPos(ImGui::GetCursorPos().x + wrap_width);
      ImGui::Text("lazy dog. This paragraph is made to fit within %.0f pixels. "
      ...
      ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(pos.x + wrap_width, pos.y),
                                                ...
                                                0xFFFF00FF);
      ImGui::PushTextWrapPos(ImGui::GetCursorPos().x + wrap_width);
      ImGui::Text("aaaaaaaa bbbbbbbb, cccccccc,dddddddd. eeeeeeee   ffffffff. "
                  "gggggggg!hhhhhhhh");
      ImGui::GetWindowDrawList()->AddRect(ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), 0xFF00FFFF);
      ImGui::PopTextWrapPos();
    ...
    if (ImGui::TreeNode("UTF-8 Text")) {
      ...
      // Most compiler appears to support UTF-8 in source code (with Visual
      // Studio you need to save your file as 'UTF-8 without signature')
      // However for the sake for maximum portability here we are *not*
      // including raw UTF-8 character in this source file, instead we encode
      // the string with hexadecimal constants.
      // In your own application be reasonable and use UTF-8 in source or
      // retrieve the data from file system!
      // Note that characters values are preserved even if the font cannot be
      // displayed, so you can safely copy & paste garbled characters into
      // another application.
      ImGui::TextWrapped("CJK text will only appears if the font was loaded "
                         "with the appropriate CJK character ranges. Call "
    ...
    if (ImGui::TreeNode("Images")) {
      ...
      ImGui::Image(tex_id, ImVec2(tex_w, tex_h), ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, 255),
    ...
    static int item = 1;
    ImGui::Combo("combo", &item, "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0"); // Combo using values
                                                                      // packed in a single
                                                                      // constant string (for
                                                                      // really quick combo)
    ...
    static int  item2 = -1;
    ImGui::Combo("combo scroll", &item2, items, IM_ARRAYSIZE(items)); // Combo using proper array. You can also
                                                                      // pass a callback to retrieve array
                                                                      // value, no need to create/copy an array
                                                                      // just for that.

    {
      ...
      ImGui::SameLine();
      ShowHelpMarker("You can apply arithmetic operators +,*,/ on numerical "
  ...
  if (ImGui::CollapsingHeader("Graphs widgets")) {
    ...
    if (animate) {
      static float refresh_time = ImGui::GetTime(); // Create dummy data at
                                                    // fixed 60 hz rate for the
                                                    // demo
  ...
  if (ImGui::CollapsingHeader("Layout")) {
    if (ImGui::TreeNode("Child regions")) {
      ...
      ImGui::PopItemWidth();
      ImGui::BeginChild("Sub1", ImVec2(ImGui::GetWindowContentRegionWidth() * 0.5f, 300), false,
    ...
    if (ImGui::TreeNode("Groups")) {
      ImGui::TextWrapped("(Using ImGui::BeginGroup()/EndGroup() to layout "
                         "items. BeginGroup() basically locks the horizontal "
                         "position. EndGroup() bundles the whole group so that "
                         "you can use functions such as IsItemHovered() on "
                         "it.)");
      ImGui::BeginGroup();
      {
        ImGui::BeginGroup();
        ...
        ImGui::Button("DDD");
        ImGui::EndGroup();
        if (ImGui::IsItemHovered()) ImGui::SetTooltip("Group hovered");
        ImGui::SameLine();
        ImGui::Button("EEE");
        ImGui::EndGroup();
      }
    ...
    if (ImGui::TreeNode("Text Baseline Alignment")) {
      ImGui::TextWrapped("(This is testing the vertical alignment that occurs "
                         "on text to keep it at the same baseline as widgets. "
                         "Lines only composed of text or \"small\" widgets fit "
                         "in less vertical spaces than lines with normal "
                         "widgets)");
      ...
      ImGui::AlignFirstTextHeightToWidgets();      // Vertically align text node a
                                                   ...
                                                   // fit).
      bool node_open = ImGui::TreeNode("Node##2"); // Common mistake to avoid: if we want to
    ...
    if (ImGui::TreeNode("Horizontal Scrolling")) {
      ImGui::Bullet();
      ImGui::TextWrapped("Horizontal scrolling for a window has to be enabled "
                         "explicitly via the "
                         "ImGuiWindowFlags_HorizontalScrollbar flag.");
      ImGui::Bullet();
      ImGui::TextWrapped("You may want to explicitly specify content width by "
                         "calling SetNextWindowContentWidth() before Begin().");
      static int lines = 7;
      ImGui::SliderInt("Lines", &lines, 1, 15);
      ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 3.0f);
      ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(2.0f, 1.0f));
      ImGui::BeginChild("scrolling", ImVec2(0, ImGui::GetItemsLineHeightWithSpacing() * 7 + 30), true,
                        ImGuiWindowFlags_HorizontalScrollbar);
      for (int line = 0; line < lines; line++) {
        ...
        for (int n = 0; n < num_buttons; n++) {
          ...
          char        num_buf[16];
          const char* label =
            (!(n % 15)) ? "FizzBuzz" : (!(n % 3)) ? "Fizz" : (!(n % 5)) ? "Buzz" : (sprintf(num_buf, "%d", n), num_buf);
          float hue = n * 0.05f;
    ...
    if (ImGui::TreeNode("Clipping")) {
      ...
      ImGui::TextWrapped("On a per-widget basis we are occasionally clipping "
                         "text CPU-side if it won't fit in its frame. "
                         "Otherwise we are doing coarser clipping + passing a "
                         "scissor rectangle to the renderer. The system is "
                         "designed to try minimizing both execution and "
                         "CPU/GPU rendering cost.");
      ...
      ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), ImGui::GetFontSize() * 2.0f,
                                          ImVec2(pos.x + offset.x, pos.y + offset.y), ImColor(255, 255, 255, 255),
  ...
  if (ImGui::CollapsingHeader("Popups & Modal windows")) {
    if (ImGui::TreeNode("Popups")) {
      ImGui::TextWrapped("When a popup is active, it inhibits interacting with "
                         "windows that are behind the popup. Clicking outside "
                         "the popup closes it.");
      ...
      if (ImGui::Button("Popup Menu..")) ImGui::OpenPopup("popup from button");
      ...
      // hovering our menu here
      // would open it. This is because once a menu is active, we allow to
      // switch to a sibling menu by just hovering on it, which is the desired
    ...
    if (ImGui::TreeNode("Modals")) {
      ...
      if (ImGui::BeginPopupModal("Stacked 1")) {
        ...
        if (ImGui::Button("Another one..")) ImGui::OpenPopup("Stacked 2");
  ...
  if (ImGui::CollapsingHeader("Columns")) {
    ...
    */
    ...
    ImGui::SameLine();
    ShowHelpMarker("NB: Tree node must be poped before ending the "
  ...
  if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus")) {
    ...
    if (ImGui::TreeNode("Focus from code")) {
      ...
      ImGui::TextWrapped("Cursor & selection are preserved when refocusing "
                         "last used item in code.");
      ImGui::TreePop();
    }

    if (ImGui::TreeNode("Dragging")) {
      ImGui::TextWrapped("You can use ImGui::GetItemActiveDragDelta() to query "
      ...
      if (ImGui::IsItemActive()) {
        ...
        draw_list->AddLine(ImGui::CalcItemRectClosestPoint(ImGui::GetIO().MousePos, true, -2.0f),
                           ImGui::GetIO().MousePos, ImColor(ImGui::GetStyle().Colors[ImGuiCol_Button]), 4.0f);
        ...
        ImGui::Text("Raw (%.1f, %.1f), WithLockThresold (%.1f, %.1f), "
                    "MouseDelta (%.1f, %.1f)",
                    value_raw.x, value_raw.y, value_with_lock_threshold.x, value_with_lock_threshold.y, mouse_delta.x,
                    mouse_delta.y);
      }
      ImGui::TreePop();
    }
    ...
    if (ImGui::TreeNode("Keyboard & Mouse State")) {
      ...
      ImGui::Text("KeyMods: %s%s%s%s", io.KeyCtrl ? "CTRL " : "", io.KeyShift ? "SHIFT " : "", io.KeyAlt ? "ALT " : "",
                  io.KeySuper ? "SUPER " : "");
    ...
    if (ImGui::TreeNode("Mouse cursors")) {
      ImGui::TextWrapped("Your application can render a different mouse cursor "
                         "based on what ImGui::GetMouseCursor() returns. You "
                         "can also set io.MouseDrawCursor to ask ImGui to "
                         "render the cursor for you in software.");
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
    ...
    if (ImGui::Button("Copy Colors")) {
      ...
      for (int i = 0; i < ImGuiCol_COUNT; i++) {
        ...
        if (!output_only_modified ||
            memcmp(&col, (ref ? &ref->Colors[i] : &default_style.Colors[i]), sizeof(ImVec4)) != 0)
          ImGui::LogText("style.Colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, "
                         "%.2ff, %.2ff);" IM_NEWLINE,
  ...
  if (ImGui::TreeNode("Fonts", "Fonts (%d)", ImGui::GetIO().Fonts->Fonts.Size)) {
    ImGui::SameLine();
    ShowHelpMarker("Tip: Load fonts with "
    ...
    for (int i = 0; i < atlas->Fonts.Size; i++) {
      ...
      ImGui::BulletText("Font %d: \'%s\', %.2f px, %d glyphs", i, font->ConfigData ? font->ConfigData[0].Name : "",
      ...
      if (ImGui::TreeNode("Details")) {
        ...
        for (int config_i = 0; config_i < font->ConfigDataCount; config_i++) {
          ...
          ImGui::BulletText("Input %d: \'%s\'\nOversample: (%d,%d), PixelSnapH: %d", config_i, cfg->Name,
...
static void ShowExampleAppAutoResize(bool* p_open) {
  ...
  static int lines = 10;
  ImGui::Text("Window will resize every-frame to the size of its "
              "content.\nNote that you probably don't want to query the window "
...
static void ShowExampleAppConstrainedResize(bool* p_open) {
                           ...
                           // constraints
  {
    static void Square(ImGuiSizeConstraintCallbackData* data) {
      data->DesiredSize =
        ImVec2(IM_MAX(data->DesiredSize.x, data->DesiredSize.y), IM_MAX(data->DesiredSize.x, data->DesiredSize.y));
    }
  ...
  if (ImGui::Begin("Example: Constrained Resize", p_open)) {
    const char* desc[] = {
      "Resize vertical only", "Resize horizontal only", "Width > 100, Height > 100",
...
static void ShowExampleAppManipulatingWindowTitle(bool*) {
  ...
  // You can use the "##" and "###" markers to manipulate the display/ID. Read
  // FAQ at the top of this file!
...
static void ShowExampleAppCustomRendering(bool* p_open) {
  ...
  // geometrical types and benefit of overloaded operators, etc.
  // Define IM_VEC2_CLASS_EXTRA in imconfig.h to create implicit conversions
  // between your types and ImVec2/ImVec4.
  // ImGui defines overloaded operators but they are internal to imgui.cpp and
  // not exposed outside (to avoid messing with your types)
  ...
  ImGui::ColorEdit3("Color", &col.x);
  {
    ...
    for (int n = 0; n < 2; n++) {
      ...
      x += sz + spacing;
      draw_list->AddTriangle(ImVec2(x + sz * 0.5f, y), ImVec2(x + sz, y + sz - 0.5f), ImVec2(x, y + sz - 0.5f), col32,
                             thickness);
      x += sz + spacing;
    ...
    draw_list->AddTriangleFilled(ImVec2(x + sz * 0.5f, y), ImVec2(x + sz, y + sz - 0.5f), ImVec2(x, y + sz - 0.5f),
                                 col32);
    x += sz + spacing;
    draw_list->AddRectFilledMultiColor(ImVec2(x, y), ImVec2(x + sz, y + sz), ImColor(0, 0, 0), ImColor(255, 0, 0),
  ...
  ImGui::Separator();
  {
    ...
    // cursor and 2) allows us to use IsItemHovered()
    // However you can draw directly and poll mouse/keyboard by yourself. You
    // can manipulate the cursor using GetCursorPos() and SetCursorPos().
    // If you only use the ImDrawList API, you can notify the owner window of
    // its extends by using SetCursorPos(max).
    ...
    for (int i = 0; i < points.Size - 1; i += 2)
      draw_list->AddLine(ImVec2(canvas_pos.x + points[i].x, canvas_pos.y + points[i].y),
                         ImVec2(canvas_pos.x + points[i + 1].x, canvas_pos.y + points[i + 1].y), 0xFF00FFFF, 2.0f);
    draw_list->PopClipRect();
...
// For the console example, here we are using a more C++ like approach of
// declaring a class to hold the data and the functions.
struct ExampleAppConsole {
  ...
  void Draw(const char* title, bool* p_open) {
    ...
    ImGui::TextWrapped("This example implements a console with basic coloring, "
                       "completion and history. A more elaborate "
                       "implementation may want to store entries along with "
                       "extra data such as timestamp, emitter, etc.");
    ...
    for (int i = 0; i < Items.Size; i++) {
      ...
      ImVec4 col = ImVec4(1.0f, 1.0f, 1.0f, 1.0f); // A better implementation
                                                   // may store a type per-item.
                                                   // For the sample let's just
                                                   // parse the text.
  ...
  static int TextEditCallbackStub(ImGuiTextEditCallbackData* data) // In C++11 you are
                                                                   // better off using
                                                                   // lambdas for this sort
                                                                   // of forwarding
                                                                   // callbacks
  {
...
static void ShowExampleAppLayout(bool* p_open) {
  ...
  if (ImGui::Begin("Example: Layout", p_open, ImGuiWindowFlags_MenuBar)) {
    ...
    // right
    ImGui::BeginGroup();
    ImGui::BeginChild("item view", ImVec2(0, -ImGui::GetItemsLineHeightWithSpacing())); // Leave room for 1
                                                                                        // line below us
...
static void ShowExampleAppPropertyEditor(bool* p_open) {
  ...
  ShowHelpMarker("This example shows how you may implement a property editor "
                 "using two columns.\nAll objects/fields data are dummies "
...
static void ShowExampleAppLongText(bool* p_open) {
  ...
  ImGui::Combo("Test type", &test_type, "Single call to TextUnformatted()\0Multiple calls to Text(), "
