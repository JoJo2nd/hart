...
static const VertexElement debugVtxDecl[] = {
  {Semantic::Position, SemanticType::Float, 3, false}, {Semantic::Color0, SemanticType::Uint8, 4, true},
};
struct DebugVtx {
  float    p[3];
  uint32_t c;
...
VertexDecl* createVertexDecl(VertexElement const* elements, uint16_t element_count) {
  ...
  for (uint16_t i = 0; i < element_count; ++i) {
    vd->decl.add(bgfxAttribRemap[(uint32_t)elements[i].sem], elements[i].elementCount,
...
Program createProgram(Shader* vertex, Shader* pixel) {
  Program p =
    bgfx::createProgram(vertex->getShaderProfileObject(currentProfile), pixel->getShaderProfileObject(currentProfile));
  return p;
}
...
void initialise(SDL_Window* window) {
  ...
  bgfx::renderFrame(); // calling this before bgfx::init prevents the render
                       // thread being created
  bgfx::init(bgfx::RendererType::Direct3D11);
  gfxCaps = *bgfx::getCaps();

#if HART_DEBUG_INFO
  debug::currentDebugPrims.vtxDecl =
    createVertexDecl(debug::debugVtxDecl, (uint16_t)HART_ARRAYSIZE(debug::debugVtxDecl));
  debug::currentDebugPrims.tvtxDecl =
    createVertexDecl(debug::debugTexVtxDecl, (uint16_t)HART_ARRAYSIZE(debug::debugTexVtxDecl));
  debug::currentDebugPrims.vtxBuffer = createVertexBuffer(nullptr, debug::MaxDebugPrimsVtxs * debug::debugVtxDeclSize,
                                                          debug::currentDebugPrims.vtxDecl, Flag_VertexBuffer_Dynamic);
  debug::currentDebugPrims.tvtxBuffer =
    createVertexBuffer(nullptr, debug::MaxDebugPrimsVtxs * debug::debugTexVtxDeclSize,
                       debug::currentDebugPrims.tvtxDecl, Flag_VertexBuffer_Dynamic);
...
void inlineBatchSubmit(uint16_t ib_offset, uint32_t ib_count, uint16_t vb_offset, uint16_t vb_count) {
  hdbassert(drawCtx.doingInlineSubmit, "inlineBatchSubmit must be called "
                                       "between beginInlineBatch & "
                                       "endInlineBatch");
...
void flushAndSumbitDebugPrims(uint16_t view, MaterialSetup* mat, hMat44 const* view_mtx, hMat44 const* proj_mtx) {
  ...
  if (!((uintptr_t)vb - (uintptr_t)vb_start)) return;

  bgfx::updateDynamicVertexBuffer(currentDebugPrims.vtxBuffer.dyvb, 0,
                                  bgfx::copy(vb_start, (uint32_t)((uintptr_t)vb - (uintptr_t)vb_start)));
  ...
  for (auto const& i : drawCalls) {
    if (i.t == DebugPrimType::Line)
      bgfx::setState(BGFX_STATE_RGB_WRITE | BGFX_STATE_ALPHA_WRITE | BGFX_STATE_MSAA | BGFX_STATE_PT_LINES |
                     BGFX_STATE_BLEND_ALPHA);
    else if (i.t == DebugPrimType::Quad)
      bgfx::setState(BGFX_STATE_RGB_WRITE | BGFX_STATE_ALPHA_WRITE | BGFX_STATE_MSAA | BGFX_STATE_BLEND_ALPHA);
