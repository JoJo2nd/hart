...
void Graph::kick() {
  for (auto& i : tasks) {
    ...
    else                                // As this task waits on others, set this value to something that'll
                                        // ensure it's never sent to a worker. Allows for
...
static uint32_t schedulerProcess(void* null) {
  ...
  while (!schedulerKillSemphore.poll()) {
    ...
    // Check the input queue for new graphs to process. (i.e. Graph.kick()
    // called)
    Graph* graph_ptr;
  ...
  while (1) {
    ...
    if (lfds_queue_dequeue(workerQueue, (void**)&task)) {
      ...
      hdbassert(task_index <= hatomic::atomicGet(task->toSend),
                "task_index is invalid. To high compared to number of tasks "
                "expected to run.");
      Info info;
      ...
      if (complete_index == hatomic::atomicGet(task->toSend)) { // Was this the last task of this
                                                                // type that needed to be run.
        if (HART_DEBUG_TASK_ORDER)
