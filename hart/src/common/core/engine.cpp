...
struct Context {
  ...
  void imguiRender(ImDrawData* draw_data) {
    ...
    for (int32_t ii = 0, num = draw_data->CmdListsCount; ii < num; ++ii) {
      ...
      hrnd::beginInlineBatch(imgui.vDecl, (void*)drawList->IdxBuffer.begin(), numIndices,
                             (void*)drawList->VtxBuffer.begin(), numVertices);
  ...
  int run(int _argc, char** _argv, GameInterface* game) {
    ...
    m_window = SDL_CreateWindow(hconfigopt::getStr("window", "title", "---"), SDL_WINDOWPOS_UNDEFINED,
    ...
    hresmgr::initialise();
    hin::initialise();
    hart::tasks::scheduler::initialise(hconfigopt::getInt("taskgraph", "workercount", 4),
                                       hconfigopt::getUint("taskgraph", "jobqueuesize", 256));

    // Application init
    static uint32_t buttom_remap[] = {1, 0, 2, 3, 4};
    engineEvents[(uint32_t)EgEvent::MouseMove] =
      addEventHandler(Event::MouseMotion, [&](Event evt_id, EventData const* evt) {
        mouseX = evt->motion.x;
        mouseY = evt->motion.y;
      });
    engineEvents[(uint32_t)EgEvent::MouseBtnDown] =
      addEventHandler(Event::MouseButtondown, [&](Event evt_id, EventData const* evt) {
        mouseButtons[buttom_remap[evt->button.button]] = true;
      });
    engineEvents[(uint32_t)EgEvent::MouseBtnUp] =
      addEventHandler(Event::MouseButtonup, [&](Event evt_id, EventData const* evt) {
        mouseButtons[buttom_remap[evt->button.button]] = false;
      });
    engineEvents[(uint32_t)EgEvent::MouseWheel] =
      addEventHandler(Event::MouseWheel, [&](Event evt_id, EventData const* evt) { wheelDelta = evt->wheel.y; });
    auto keyProc = [&](Event evt_id, EventData const* evt) {
      ImGuiIO& io = ImGui::GetIO();
    ...
    engineEvents[(uint32_t)EgEvent::KeyUp] = addEventHandler(Event::KeyUp, keyProc);
    engineEvents[(uint32_t)EgEvent::KeyDown] = addEventHandler(Event::KeyDown, keyProc);
    engineEvents[(uint32_t)EgEvent::TextInput] =
      addEventHandler(Event::TextInput, [&](Event evt_id, EventData const* evt) {
        ImGuiIO& io = ImGui::GetIO();
    ...
    */
    ...
    while (!sys_collection_hdl.loaded())
      hresmgr::update();
