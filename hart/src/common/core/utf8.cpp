...
uint32_t decodeToUnicode( const hChar* hRestrict uft8In, Unicode& ucOut )
{
    ...
    {
        ucOut = ( ( uft8In[ ret+1 ] & 0x1F ) << 6 ) | ( uft8In[ ret+2 ] &
MASKBITS );
    }
    // 0xxxxxxx
...
uint32_t encodeFromUnicodeString(const Unicode* hRestrict ucin, uint32_t
inlimit, hChar* hRestrict utf8out, uint32_t outlimit) {
    uint32_t written=0;
