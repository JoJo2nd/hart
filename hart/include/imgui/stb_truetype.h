...
//   typedef
//   1.09 (2016-01-16) warning fix; avoid crash on outofmem; use allocation
//   userdata properly
//   1.08 (2015-09-13) document stbtt_Rasterize(); fixes for vertical &
//   horizontal edges
//   1.07 (2015-08-01) allow PackFontRanges to accept arrays of sparse
//   codepoints;
//                     variant PackFontRanges to pack and render in separate
//                     phases;
//                     fix stbtt_GetFontOFfsetForIndex (never worked for non-0
//                     input?);
...
//           use as texture
//           stbtt_GetBakedQuad()                 -- compute quad to draw for a
//           given char
//
...
//           want it
//           stbtt_PackBegin()
//           stbtt_PackSetOversample()            -- for improved quality on
//           small fonts
...
//           bitmap
//           stbtt_MakeCodepointBitmap()          -- renders into bitmap you
//           provide
...
STBTT_DEF int stbtt_PackFontRanges(stbtt_pack_context* spc, unsigned char* fontdata, int font_index,
...
STBTT_DEF int stbtt_PackFontRangesGatherRects(stbtt_pack_context* spc, stbtt_fontinfo* info, stbtt_pack_range* ranges,
                                              int num_ranges, stbrp_rect* rects);
STBTT_DEF void stbtt_PackFontRangesPackRects(stbtt_pack_context* spc, stbrp_rect* rects, int num_rects);
STBTT_DEF int stbtt_PackFontRangesRenderIntoRects(stbtt_pack_context* spc, stbtt_fontinfo* info,
...
// so you should advance the vertical position by "*ascent - *descent +
// *lineGap"
...
// advanceWidth is the offset from the current horizontal position to the next
// horizontal position
...
STBTT_DEF void stbtt_GetBakedQuad(stbtt_bakedchar* chardata, int pw, int ph, int char_index, float* xpos, float* ypos,
...
STBTT_DEF void stbtt_GetPackedQuad(stbtt_packedchar* chardata, int pw, int ph, int char_index, float* xpos, float* ypos,
...
static stbtt_int32 stbtt__CompareUTF8toUTF16_bigendian_prefix(const stbtt_uint8* s1, stbtt_int32 len1,
        ...
        if (matchlen >= 0) {
          ...
          if (i + 1 < count && ttUSHORT(fc + loc + 12 + 6) == next_id && ttUSHORT(fc + loc + 12) == platform &&
            ...
            if (slen == 0) {
              ...
              ++matchlen;
              if (stbtt_CompareUTF8toUTF16_bigendian((char*)(name + matchlen), nlen - matchlen,
...
//   1.03 (2015-04-12) STBTT_STATIC, fix memory leak in new packing, various
//   fixes
//   1.02 (2014-12-10) fix various warnings & compile issues w/ stb_rect_pack,
//   C++
