...
#define LFDS_PLATFORM_WINDOWS (1)
#else
#error "Unknown platform"
#endif

#if HART_64BIT
#define LFDS_BUILD_64_BIT (1)
#elif HART_32BIT
#define LFDS_BUILD_32_BIT (1)
#else
#error "Unknown word size"
#endif

/***** defines *****/
#if (defined LFDS_PLATFORM_WINDOWS && defined LFDS_BUILD_64_BIT && !defined WIN_KERNEL_BUILD)
...
#define _InterlockedCompareExchangePointer(destination, exchange, compare)                                             \
  _InterlockedCompareExchange((volatile long*)destination, (long)exchange, (long)compare)
#endif
...
#if (defined LFDS_PLATFORM_LINUX && !defined LFDS_BUILD_64_BIT && defined WIN_KERNEL_BUILD)
...
#if (defined LFDS_PLATFORM_LINUX && defined LFDS_BUILD_64_BIT)
// TRD : any UNIX on x64
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
...
#define LFDS_BARRIER_LOAD                                                                                              \
  LFDS_BARRIER_COMPILER_LOAD;                                                                                          \
  LFDS_BARRIER_PROCESSOR_LOAD;                                                                                         \
  LFDS_BARRIER_COMPILER_LOAD
#define LFDS_BARRIER_STORE                                                                                             \
  LFDS_BARRIER_COMPILER_STORE;                                                                                         \
  LFDS_BARRIER_PROCESSOR_STORE;                                                                                        \
  LFDS_BARRIER_COMPILER_STORE
#define LFDS_BARRIER_FULL                                                                                              \
  LFDS_BARRIER_COMPILER_FULL;                                                                                          \
  LFDS_BARRIER_PROCESSOR_FULL;                                                                                         \
  LFDS_BARRIER_COMPILER_FULL
...
enum lfds_freelist_query_type { LFDS_FREELIST_QUERY_ELEMENT_COUNT, LFDS_FREELIST_QUERY_VALIDATE };
