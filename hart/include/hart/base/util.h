...
constexpr size_t arraySize(const t_ty (&)[n]) {
  return n;
}
...
inline uint32_t endianSwap(uint32_t v) {
  return ((uint32_t)endianSwap(uint16_t(v & 0x0000FFFF)) << 16) |
         ((uint32_t)endianSwap(uint16_t((v & 0xFFFF0000) >> 16)));
}

inline uint64_t endianSwap(uint64_t v) {
  return ((uint64_t)endianSwap(uint32_t(v & 0x00000000FFFFFFFF)) << 32) |
         ((uint64_t)endianSwap(uint32_t((v & 0xFFFFFFFF00000000) >> 32)));
}
...
#define HART_MAKE_FOURCC(a, b, c, d)                                                                                   \
