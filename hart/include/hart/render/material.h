...
public:
  ...
  char const* getInputParameterName(MaterialInputHandle h) { return inputs[h.idx].name; }
  ...
  void setParameters(MaterialHandleData const* hrestrict in_data, uint32_t in_data_count,
...
private:
  uint32_t getTechnqiueIndex(TechniqueType in_type) {
    for (uint32_t i = 0, n = (uint32_t)techniques.size(); i < n; ++i) {
      if (techniques[i].type == in_type) return i;
    }
    return ~0ul;
  }
