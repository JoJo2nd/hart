...
struct HandleBase {
  ...
  bool valid() const { return !huuid::isNull(id); }
...
struct THandle : public HandleBase {
  t_ty* getData() { return (t_ty*)getDataRaw(t_ty::getTypeCC()); }

  t_ty* operator->() { return getData(); }
};
...
// Use when it is know that the resource will not unload due to some other
// dependency
