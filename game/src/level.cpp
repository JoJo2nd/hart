...
bool Level::deserialiseObject(MarshallType const* in_data, hobjfact::SerialiseParams const& params) {
      ...
      for (uint32_t x = 0; x < l_width; x += 32) {
        for (uint32_t tsi = 0, tsn = (uint32_t)tilesets.size(); tsi < tsn; ++tsi) {
          ...
          for (uint32_t yy = 0; yy < 32; ++yy) {
            for (uint32_t xx = 0; xx < 32; ++xx) {
              ...
              if (Tileset::Tile const* t = getTile(t_data[loc], &tw, &th, &ts)) {
                ...
                lv_ptr[0].x = (float)(x + xx) * tw;
                lv_ptr[0].y = (float)(y + yy) * th;
                lv_ptr[0].u = t->uvs[0];
                lv_ptr[0].v = t->uvs[1];
                lv_ptr[1].x = (float)(x + xx + 1) * tw;
                lv_ptr[1].y = (float)(y + yy) * th;
                lv_ptr[1].u = t->uvs[2];
                lv_ptr[1].v = t->uvs[1];
                lv_ptr[2].x = (float)(x + xx) * tw;
                lv_ptr[2].y = (float)(y + yy + 1) * th;
                lv_ptr[2].u = t->uvs[0];
                lv_ptr[2].v = t->uvs[3];
                lv_ptr[3].x = (float)(x + xx + 1) * tw;
                lv_ptr[3].y = (float)(y + yy + 1) * th;
                lv_ptr[3].u = t->uvs[2];
                lv_ptr[3].v = t->uvs[3];
              }
            }
          ...
          if (cur_ts && animquads > 0) {
            hrnd::IndexBuffer qib = hrnd::createIndexBuffer(tmp_anim_ib, animquads * indexCount * sizeof(uint16_t),
                                                            hrnd::Flag_IndexBuffer_Dynamic);
            hrnd::VertexBuffer qvb = hrnd::createVertexBuffer(tmp_anim_vb, animquads * vtxCount * sizeof(Vtx), vDecl,
                                                              hrnd::Flag_VertexBuffer_Dynamic);
  ...
  for (uint32_t i = 0, n = col_prims->size(); i < n; ++i) {
    if ((*col_prims)[i]->points()) {
      ...
      AABB col_quad((*col_prims)[i]->aabb_min()->x(), (*col_prims)[i]->aabb_min()->y(),
                    (*col_prims)[i]->aabb_max()->x(), (*col_prims)[i]->aabb_max()->y());
      collisionQuads.push_back(collisionworld::addStaticPrimitive(col_quad));
    }
  }
    ...
    for (auto& anim : i.sprites) {
      ...
      if (anim.playTicks >= (*anim.seq)[anim.frame].tickLen) {
        ...
        if (Tileset::Tile const* t = &i.tileset->tiles[nfid]) {
          vb_ptr[basei + 0].u = t->uvs[0];
          vb_ptr[basei + 0].v = t->uvs[1];
          vb_ptr[basei + 1].u = t->uvs[2];
          vb_ptr[basei + 1].v = t->uvs[1];
          vb_ptr[basei + 2].u = t->uvs[0];
          vb_ptr[basei + 2].v = t->uvs[3];
          vb_ptr[basei + 3].u = t->uvs[2];
          vb_ptr[basei + 3].v = t->uvs[3];
        }
        dirty = true;
      }
    }
