...
HART_OBJECT_TYPE_DECL_CUSTOM(Sprite, []() -> void* { return spriteFreelist.allocate(); },
                             [](void* ptr) { spriteFreelist.release(ptr); },
                             hobjfact::typehelper_t<Sprite>::constructType,
                             hobjfact::typehelper_t<Sprite>::destructType, nullptr);
  ...
  hart::engine::addDebugMenu("Sprite Viewer", []() {
    ...
    if (ImGui::Begin("Sprite Viewer", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_MenuBar)) {
      if (loadedSprites.size()) {
        ImGui::ListBox("Loaded Sprites", &current_sprite,
      ...
      if (current_sprite < loadedSprites.size()) {
        ...
        if (anim_frames.size()) {
          ...
          ImGui::Image(tex_id, ImVec2((float)preview_size, (float)preview_size), ImVec2(frame.uvs[0], frame.uvs[1]),
                       ImVec2(frame.uvs[2], frame.uvs[3]));
          anim_time += delta_ms;
